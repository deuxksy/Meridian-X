# Meridian-X Documentation Hub

Meridian-X 프로젝트의 문서 집합입니다. Diátaxis 프레임워크 4분면(Tutorials, How-To, Reference, Explanation)에 따라 정리되어 있습니다.

---

## 📚 Diátaxis 문서 분류 (Diátaxis Index)

### 1. 🚀 Tutorials (튜토리얼)
- [Quick Start Guide](../README.md#-사용법-usage): 설치, 설정 및 파이프라인 첫 실행 가이드

### 2. 💡 How-To Guides (사용법 가이드)
- [CLI 사용 설명서](../README.md#-주요-기능-features): `collect`, `filter`, `label`, `sync`, `tidy`, `classify`, `pipeline`, `report` 개별 명령어 가이드
- [URL Resolver 사용법](../README.md#-사용법-usage): `url-resolver` 단축링크 우회 및 aria2 자동 전송 가이드

### 3. 📖 Reference (참조)
- [Configuration Spec](../README.md#-설정-configuration): `config/settings.json` 및 `sops` 암호화 설정 명세
- [CLI Options Reference](../README.md#-명령어-옵션): 명령줄 옵션 모음

### 4. 🧠 Explanation (설명 및 설계)
- [Project Roadmap](../ROADMAP.md): 버전별 현황 및 향후 로드맵
- [Security Scan 설계](./plans/2026-04-03-security-scan-design.md): 보안 스캔 도입 배경 및 설계 결정
- [Security Scan 구현](./plans/2026-04-03-security-scan-implementation.md): 보안 스캔 CI/CD 파이프라인 구축 계획

---

## 📁 디렉터리 안내 (Subdirectories)

- **`docs/plans/`**: 시스템 기능 설계 및 구현 계획서
- **`docs/superpowers/`**: AI 에이전트 설계 사양(`specs/`) 및 실행 계획(`plans/`) (내부 관리용)
