# `mx` (Meridian-X Direct Link Extractor & Aria2 Dispatcher CLI) Design Specification

**Date:** 2026-08-09  
**Status:** Approved / Draft for Implementation  
**CLI Executable Name:** `mx`  

---

## 1. Executive Summary

`mx`는 `misskon.com` 등과 같이 복잡한 웹페이지 및 단축 링크 체인(`ouo.io`, `ouo.press`, `mediafire.com` 등)에서 대용량 파일의 **최종 직링크(Direct Download URL)**와 **Referer/User-Agent 메타데이터**를 자동으로 추출하고, 선택적으로 원격 `aria2` 서버(`ws://heritage.bun-bull.ts.net:6800`)로 전달하는 2단계 파이프라인 CLI 도구이다.
...
flowchart TD
    subgraph Inputs ["Input Sources"]
        CLI_URL["mx parse <URL>"]
        CLI_CLIP["mx clip"]
        CLI_BATCH["mx batch <file.txt>"]
    end
...
### 6.4 `Aria2Dispatcher`
- `~/.config/mx/config.toml`에서 aria2 호스트 (`ws://heritage.bun-bull.ts.net:6800`) 및 secret 토큰 정보 읽기.
...
└── superpowers/
    └── specs/
        └── 2026-08-09-mx-url-resolver-design.md
```
